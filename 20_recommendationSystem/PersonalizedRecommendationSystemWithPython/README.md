...
# Surprise
```
특정 도메인에 대해서 추천시스템을 적용할때 어떤 알고리즘이 좋을지, 어떤 설정이 좋을지 세팅할때 유용하게 사용된다 
개발 초기에 탐색하기에 좋은 패키지 
```
...
# 하이브리드 추천 시스템 
```
여기까지 하나의 알고리즘을 사용해서 예측값을 구하는 방법을 배워봄
하지만 실제로는 여러가지 알고리즘을 결합하는 경우가 많음 
이유는 하나로 하는것보다 정확한 경우가 많았기 때문임
복수의 알고리즘을 사용하는 추천시스템을 하이브리드 추천 시스템이라고 한다 

한 알고리즘의 오류를 다른 알고리즘이 보정하고 찾지 못한 패턴을 보정해준다 

data 에 있는 데이터셋 기준으로 
MF   CF 비중 
0.87 0.13
이 가장 좋은 효율로 보인다 0.9089340

```


# 대규모 데이터 처리를 위한 Sparse matrix
```
데이터가 많을때 메모리의 한계로 
전체 데이터를 올려서 분석해야하는데 불가능한 경우가 있음 
추천시스템에서는 대부분의 비어있는 희박행렬이 많다. 
전체 데이터에서 0.1%만 차있다. 그 우리 data에 있는 평점데이터의 경우에는

다뤄야할 데이터가 희박행렬이라면 전체행렬을 저장하는 대신에 행렬의 원소중에서 실제 데이터를 가지는것만 그 인덱스와 같이 저장하는 편이 효율적
scipy.sparse import csr_matrix 를 구현해놓았다 
...
```
# 신규 사용자와 신규상품의 등장했을때
 - 처음에는 인기제품이나 랜덤으로 추천해주는식으로 데이터를 쌓아야함 
# 확장성
 - Spark같은 병렬처리 기술을 사용 
 - 실시간으로 계산을 바로바로 하는것이 불가능할때가 많다 -> 한가할때 주요 계산 배치작업 (사용자별 추천리스트 )
 - 데이터가 너무 많으면 item based cf 를 사용, 또는 다른 알고리즘 사용 
# 이진수 데이터 
 - 자카드 유사도 를 사요8ㅇ하거나 하는 식의 변형이 필요함 
# 간접평가 데이터 
